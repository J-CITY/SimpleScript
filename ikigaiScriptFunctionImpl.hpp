				...
				for (auto&& sub : subexpressions) {
					if (sub->type == ExpressionType::Return) {
						...
						if (result->type == ExpressionType::Return) {
							returnVal = std::static_pointer_cast<ValueNode>(result)->value;
							break;
						}
					}
				}
...
FunctionRef IkigaiScriptInterpreter::newFunction(
	...
	return newFunction(name, scope, std::make_shared<Function>(name, argNames));
}
...
FunctionRef IkigaiScriptInterpreter::newConstructor(
	...
	return newConstructor(name, scope, std::make_shared<Function>(name, argNames));
}
