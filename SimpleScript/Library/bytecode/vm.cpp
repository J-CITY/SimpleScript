    ...
    while (frames_.size() > stopDepth) {
        ...
        case OpCode::OP_GET_MEMBER: {
            ...
            if (obj->getType() == Type::Class) {
                ...
                if (it != cls->variables.end()) { push(it->second); break; }
            }
        ...
        case OpCode::OP_NEG: {
            ...
            if (fnc) { push(interp_->callFunction(fnc, scope, args)); break; }
            // Fallback: inline negation
            if (a->getType() == Type::Int)   { push(std::make_shared<Value>(-a->getInt())); break; }
            if (a->getType() == Type::Float) { push(std::make_shared<Value>(-a->getFloat())); break; }
