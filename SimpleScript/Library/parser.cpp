...
namespace IkigaiScript {
	...
	bool needsUnaryPlacementFix(const std::vector<std::string_view>& strings, size_t i) {
		return (isUnaryMathOperator(strings[i]) && (i == 0 || !(isClosingBracketOrParen(strings[i - 1]) ||
			isVarOrFuncToken(strings[i - 1]) || isNumeric(strings[i - 1]))));
	}

	bool checkPrecedence(OperatorPrecedence curr, OperatorPrecedence neww) {
		return (int)curr < (int)neww || (neww == curr && (neww == OperatorPrecedence::incdec || neww ==
	...
	ExpressionPtr Parser::getExpression(std::vector<std::string_view> strings, ScopePtr scope, Class* classs) {
		auto parseArg = [&](std::vector<std::string_view> sub) -> ExpressionPtr
		{
			if (sub.size() >= 3 && isVarOrFuncToken(sub[0]) && sub[1] == "=") {
				...
				return interpreter->arena.make<NamedArgumentExpression>(
					name, getExpression(std::move(rhs), scope, classs));
			}
		...
		while (i < strings.size()) {
			if (isMathOperator(strings[i])) {
				auto prev = root;
				root = interpreter->arena.make<FunctionExpression>(
					interpreter->resolveVariable(std::string(strings[i]), interpreter->modules[0].scope));
				auto curr = prev;
				if (curr) {
					...
					if (curr->type == ExpressionType::FunctionCall) {
						...
						if (funcExpr->getType() != Type::Function) {
							...
							if (curfunc && checkPrecedence(curfunc->opPrecedence, newfunc->opPrecedence)) {
								while (static_cast<FunctionExpression*>(curr)->subexpressions.back()->type ==
									ExpressionType::FunctionCall) {
									auto fnc = static_cast<FunctionExpression*>(static_cast<FunctionExpression*>(curr)->
										subexpressions.back())->function;
								...
								if (i + 1 < strings.size() && !isMathOperator(strings[i + 1])) {
									...
									rootExpression->subexpressions.push_back(
										getExpression(std::move(minisub), scope, classs));
								}
				...
				auto val = multiline
					? std::string(strings[i].substr(3, strings[i].size() - 6))
				...
				if (i + 1 < strings.size() && strings[i + 1] == "{") {
					...
					if (!safeBlockExpr->subexpressions.empty()) {
						...
						if (lastType != ExpressionType::DefineVar && lastType != ExpressionType::Return && lastType !=
							ExpressionType::Defer) {
				...
				if (strings[i] == "(" || i + 2 < strings.size() && strings[i + 1] == "(") {
					...
					if (strings[i] == "(") {
						...
						if (root) {
							if (root->type == ExpressionType::FunctionCall) {
								if (static_cast<FunctionExpression*>(root)->function->getFunction()->opPrecedence ==
									...
									static_cast<FunctionExpression*>(root)->subexpressions.push_back(
										interpreter->arena.make<FunctionExpression>(
										interpreter->identityFunctionVarLocation));
									cur = static_cast<FunctionExpression*>(root)->subexpressions.back();
								}
							}
							else {
								static_cast<MemberFunctionCall*>(root)->subexpressions.push_back(
									interpreter->arena.make<FunctionExpression>(
									interpreter->identityFunctionVarLocation));
							...
							root = interpreter->arena.make<
								FunctionExpression>(interpreter->identityFunctionVarLocation);
							cur = root;
						}
					}
					else {
					...
					while (nestLayers > 0 && ++i < strings.size()) {
						if (nestLayers == 1 && strings[i] == ",") {
							if (minisub.size()) {
								static_cast<FunctionExpression*>(cur)->subexpressions.push_back(
									parseArg(std::move(minisub)));
								minisub.clear();
							}
						}
						else if (isClosingBracketOrParen(strings[i])) {
						...
						else if (isOpeningBracketOrParen(strings[i]) || !(strings[i].size() == 1 &&
							contains("+-*%/"s, strings[i][0])) && i + 2 < strings.size() && strings[i + 1] == "(") {
							++nestLayers;
							if (strings[i] == "(") {
								minisub.push_back(strings[i]);
							}
					...
					// list
					bool indexOfIndex = i > 0 && (isClosingBracketOrParen(strings[i - 1]) || strings[i - 1].back() ==
						'\"') || (i > 1 && strings[i - 2] == ".");
					ExpressionPtr cur = nullptr;
					if (!indexOfIndex && strings[i] == "[") {
						...
						if (list.size()) {
							...
							for (auto& val : list) {
								if (val->getType() == Type::Null || val->getType() != type || (int)val->getType() >= (
									int)Type::Array) {
									canBeArray = false;
									break;
								}
						...
						// list access
						auto indexexpr = interpreter->arena.make<FunctionExpression>(
							interpreter->listIndexFunctionVarLocation);
						if (indexOfIndex) {
							cur = root;
							auto parent = root;
							while (cur->type == ExpressionType::FunctionCall
								&& static_cast<FunctionExpression*>(cur)->function->getType() == Type::Function
								&& static_cast<FunctionExpression*>(cur)->function->getFunction()->opPrecedence !=
								OperatorPrecedence::func) {
								parent = cur;
							...
							static_cast<FunctionExpression*>(cur)->subexpressions.push_back(
								getResolveVarExpression(std::string(strings[i]), parseScope->isClassScope));
							++i;
						}
				...
				if (strings[i] == "true") {
					...
					if (root) {
						if (root->type == ExpressionType::ResolveVar || root->type == ExpressionType::MemberVariable) {
							throw Exception(
								"Syntax Error: unexpected series of values at "s + std::string(strings[i]) +
								", possible missing `,`");
						}
						static_cast<FunctionExpression*>(root)->subexpressions.push_back(newExpr);
					}
				...
				if (isfunc) {
					ExpressionPtr expr;
					{
						if (root->type == ExpressionType::FunctionCall && static_cast<FunctionExpression*>(root)->
							subexpressions.size()) {
							expr = interpreter->arena.make<MemberFunctionCall>(
								static_cast<FunctionExpression*>(root)->subexpressions.back(),
								std::string(strings[++i]), std::vector<ExpressionPtr>());
							...
							expr = interpreter->arena.make<MemberFunctionCall>(
								root, std::string(strings[++i]), std::vector<ExpressionPtr>());
							root = expr;
						}
					}
					...
					while (nestLayers > 0 && ++i < strings.size()) {
						if (nestLayers == 1 && strings[i] == ",") {
							if (minisub.size()) {
								static_cast<MemberFunctionCall*>(expr)->subexpressions.push_back(
									parseArg(std::move(minisub)));
								minisub.clear();
								addedArgs = true;
							}
					...
					if (root->type == ExpressionType::FunctionCall && static_cast<FunctionExpression*>(root)->
						subexpressions.size()) {
						auto expr = interpreter->arena.make<MemberVariable>(
							static_cast<FunctionExpression*>(root)->subexpressions.back(), std::string(strings[++i]));
						static_cast<FunctionExpression*>(root)->subexpressions.pop_back();
						static_cast<FunctionExpression*>(root)->subexpressions.push_back(expr);
					}
					else {
				...
				auto newExpr = interpreter->arena.make<ValueNode>(
					ValuePtr(isFloat ? new Value((Float)val) : new Value((Int)val)), ExpressionType::Value);
				if (root) {
					static_cast<FunctionExpression*>(root)->subexpressions.push_back(newExpr);
				}
				else {
		...
		switch (parseState) {
					...
					if (!closedExpr || parseScope->name == "__anon") {
						if (parseScope->isClassScope) {
							...
							if (!parseScope->parent->functions.contains(className)) {
								...
								auto defaultCtor = std::make_shared<Function>(
									className, argNames, argTypes, defValues, bodyExprs);
						...
						switch (exprs.size()) {
							...
							case 3:
							{
								if (!exprs[0].empty()) {
									...
									loop->initExpression = interpreter->arena.make<DefineVar>(
										std::string(name), getExpression(exprs[0], parseScope, nullptr));
								}
								if (!exprs[1].empty()) {
			...
			if (token == ")") {
				if (--outerNestLayer <= 0) {
					...
					if (exprs.size() != 2) {
						clearParseStacks();
						throw Exception(
							"Syntax error, `foreach` requires 2 statements, "s + std::to_string(exprs.size()) +
					...
					static_cast<Foreach*>(currentExpression)->listExpression = getExpression(
						exprs[1], parseScope, nullptr);
			...
			if (token == ")") {
				if (--outerNestLayer <= 0) {
					...
					static_cast<IfElse*>(currentExpression)->states.back().testExpression = getExpression(
						move(parseStrings), parseScope, nullptr);
					clearParseStacks();
				}
				else {
					parseStrings.push_back(token);
						...
						if (!line.empty() && line[0] == "(") {
							...
							if (depth == 0 && closeIdx + 1 < line.size() && line[closeIdx + 1] == "=") {
								...
								}
								{
									...
									for (auto& n : patNames) {
										if (n == "_") continue;
										if (!seen.insert(n).second) throw Exception(
											"Duplicate name '" + n + "' in destructuring pattern");
									}
								}
								std::vector<std::string_view> rhs(line.begin() + closeIdx + 2, line.end());
								ExpressionPtr valExpr = rhs.empty()
									? nullptr
									: getExpression(std::move(rhs), parseScope, nullptr);
								auto astNode = interpreter->arena.make<DestructuringAssign>(
									std::move(patNames), valExpr);
								if (!currentExpression) {
			...
			if (token == ";") {
				if (currentExpression) {
					currentExpression->push_back(
						interpreter->arena.make<Yield>(getExpression(move(parseStrings), parseScope, nullptr)));
				}
				clearParseStacks();
			}
			else {
			...
			if (token == "if") {
				...
				clearParseStacks();
				throw Exception(
					"Malformed Syntax: Incorrect token `" + std::string(token) + "` following `else` keyword");
			}
			break;
			...
			if (parseLambdaBrakets == 0 && (parseLambda == 0 || parseLambda == 2) && token == ";") {
				...
				if (parseStrings[0] == "(") {
					...
					ExpressionPtr defineExpr = rhs.empty()
						? nullptr
						: getExpression(std::move(rhs), parseScope, nullptr);
				...
				if (parseStrings.size() > 2) {
					...
					if (!parseStrings.empty() && parseStrings[0] == "=") {
						...
						if (parseLambda == 2 && !parseStrings.empty() && parseStrings[0] == "fun") {
							parseLambda = 0;
							defineExpr = parseInlineLambda(
								std::vector<std::string_view>(parseStrings.begin(), parseStrings.end()), parseScope);
						}
						else {
							parseLambda = 0;
			...
			if (token == ",") {
				if (parseStrings.size()) {
					...
					parseScope->baseClasses.insert(parseScope->baseClasses.end(), otherscope->baseClasses.begin(),
						otherscope->baseClasses.end());
			...
			if (token == ";") {
				...
				if (parseStrings.size() >= 4 && parseStrings[0] == "{") {
					...
					if (closeBraceIdx == 0 || closeBraceIdx + 2 >= parseStrings.size() || parseStrings[closeBraceIdx +
						1] != "from") {
			...
			if (token == "(") {
				...
				for (int i = 0; i < parseStrings.size();) {
					if (state == ParseFuncArgs::Arg) {
						...
						if (varArgsValue) {
							//after variable nums of arg can not set enather var
							throw Exception("Malformed function argument syntax");
						}
						args.emplace_back(parseStrings[i]);
						i++;

						if (parseStrings.size() > i + 2 && parseStrings[i] == "." && parseStrings[i + 1] == "." &&
							parseStrings[+2] == ".") {
							if (parseState == ParseState::OperatorArgs) {
						...
						if (parseStrings.size() > i && parseStrings[i] == ",") {
							if (hasDefValue) {
								throw Exception("Malformed function argument syntax");
								// After default values, all remaining args must have defaults
							}
							i++;
				...
				if (isOperator) {
					...
					switch (funcType) {
						...
						case OperatorPrecedence::muldiv:
						{
							argsSize = 2;
							break;
						};
						case OperatorPrecedence::assign:;
						case OperatorPrecedence::incdec:
						{
							argsSize = 1;
							break;
						}
						case OperatorPrecedence::func:
						{
							throw Exception("Malformed function argument syntax");
						}
						default:
				...
				auto newfunc = isConstructor
					? interpreter->
					newConstructor(std::string(fncName), parseScope->parent, args, types, defValues)
					: (isOperator
					? interpreter->newOperator(std::string(fncName), parseScope, args, types,
					defValues)
					: interpreter->newFunction(std::string(fncName), parseScope, args, types,
					defValues));
			...
			case ParseState::FuncResult:
			{
				if (token == "{") {
					if (parseStrings.empty()) {
						throw Exception(
							"Malformed Syntax: expected return type annotation before `{` in function definition");
					}
					if (parseStrings[0] == ":") {
						...
						throw Exception(
							"Malformed Syntax: expected `:` before return type, got `" + std::string(parseStrings[0]) +
							"`");
					}
